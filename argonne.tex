...
Light source scientists are now faced with a data crisis as the rate at which data volumes 
are generated is rapidly exceeding Moore's Law. Growing data 
volumes present new challenges to users as they overwhelm their capabilities and require new 
automated approaches to outsource the acquisition, analysis, and distribution of the 
light source science data lifecycle. Neither humans or computers can cope using current methods and 
the existing techniques to design experiments, manage and analyze data, and create and 
deliver software will not suffice for next generation data rates. Indeed, these 
challenges present new opportunities and the potential to create the automation and 
enrichment services needed to exploit instrument advancements. New approaches are necessary to 
relieve scientists from time consuming data munging, management, and dissemination practices, 
including reliably indexing and cataloging, pipelining, and transforming results.
...
\ryan{Something along these lines?}
Here we propose automation and outsourcing as a solution to enable next generation light source 
science. We discuss areas in which we are working to reduce the burden of scientists by 
developing the tools and techniques needed to reliably manage data. First, we present data 
acquisition and distribution tools and services. We then present the case for automation and the 
techqinues used to enables its application. We then describe data indexing and search tools, such 
as the Materials Data Facility, to outsource the management of data once it has been created. 
Finally, we present the need for transformation services, such as DLHub.
...
\ryan{Not sure what to call this, but it gives this text a place.}
...
\ian{Next text and Figure~\ref{fig:diffuse} are from~\cite{foster2015networking}. Need to be 
rewritten.}
\kyle{re-written}
...
Ultimately, we want both experimental and simulation data to feed into an evolving knowledge base 
that can then play an important role in guiding current and future research. 
Automation of these processes can be achieved to simplify 
the processes used by scientists, improve throughput, and increase both the quality and accuracy 
of the results generated. 

We also note that human collaboration and guidance is important at many points and scales, from 
direct steering of experiments to the choice of the next experiment and simulation. 
Human-in-the-loop interactions are critical to both the design of experiments and analysis of 
results. Automation solutions require the capabilities to incorporate users into the process such 
that they can correct errors and optimize experiments.

Many light source experiments apply a similar process to the acquisition, analysis, and 
distribution of data. We posit that components of the light source science data 
lifecycle can be reliably outsourced to specialized services, thus reducing the burdon on 
scientists. Specialized services can be created to service many sources 
concurrently, reliably, and efficiently. These services can reliably provide the tools necessary to 
perform a wide range of advanced analyses without requiring the scientist to have specialized 
skills in terms of building, running, and maintaining analytical tools, and removing the barriers to 
entry to apply analytical tools across leadership computing resources. In addition, specialized 
services to automatically extract metadata and enforce best-practice cataloging and replication can 
be deployed such that scientists can simply invoke such a service, rather than creating ad-hoc data 
management solutions with little reproducibility, adding to the vanguard computing environment that 
litters many large-scale experimental science instruments.
...
are generated from new instruments and robotic processes increasingly being used to rapidly manage 
the analysis of multiple samples, automated acquisition processes become increasingly necessary. New 
techniques to responsively manage data as they are generated are needed. 
...
enact actions, such as data movement, in response to samples being acquired. Similarly, ISPyB is a 
...
This service, called Petrel, provides researchers access to a large, 1.5PB, store positioned within 
...
own institutes credentials.



\section{DATA PUBLICATION AND DISCOVERY}
\ryan{Making this more general with MDF and search being the main selling points.}
...
Sharma, Wozniak, and others developed an online analysis pipeline for high energy diffraction 
microscopy
experiments 
...
\section{TRANSFORMATION AND ANALYSIS}
